...
###################################################
# This File Will Be Rewrite To Add Eel To Project #
###################################################
...
def clickHandler(event, canvas):
    ...
    if conf.mode == ConfigModes.normal:
        if can_draw_line:
            ...
            current_line.start_box = canvas.find_withtag(
                canvas.gettags(draw_object)[0].replace("exeout", ""))
...
def movingMousePressed(event, canvas, app):
        ...
        if closest[0] not in locked_image_objects:
            if len(closest_tag[0]) != "temp":
                if closest_tag[0].endswith("exein") or closest_tag[0].endswith("exeout"):
                    try:
                        canvas.itemconfig(
                            images[closest_tag[0]], image=app.full_execute_image)
                        ...
                        try:
                            canvas.itemconfig(
                                images[closest_tag[0]], image=app.empty_execute_image)
                            can_draw_line = False
                            draw_object = None
                        except:
                            pass
    ...
    if current_line.is_drawing:
        canvas.delete("temp")
        canvas.create_line(current_line.start[0], current_line.start[1],
                           event.x, event.y, tags="temp", width=4, fill='white')
    else:
                ...
                if len(closest_tag) == 2 and closest_tag[1] == 'current':
                    ...
                    try:
                        for i in images:
                            ...
                            if var[0] not in locked_image_objects:
                                canvas.itemconfig(
                                    images[i], image=app.empty_execute_image)
                        can_draw_line = False
                        draw_object = None
                    except:
                        pass
    ...
    if not is_dbl_click:
        if conf.mode == ConfigModes.normal:
            if can_draw_line:
                ...
                current_line.end_box = canvas.find_withtag(
                    canvas.gettags(draw_object)[0].replace("exein", ""))
                locked_image_objects.append(draw_object)
                canvas.delete("temp")
                canvas.create_line(current_line.start[0], current_line.start[1], current_line.end[0],
                                   current_line.end[1], tags=current_line.tag, width=4, fill='white')
