...
@eel.expose
def make_box_connector(box_type, first_index, second_index=None, from_extention=False):
    box = None
    if box_type == 1:
        ...
        if from_extention:
            box_exact = box_categories[tp][MainBoxTypes.Extentions][executalbe_categories[first_index]][second_index]
        ...
        if from_extention:
            box_exact = box_categories[tp][MainBoxTypes.Extentions][first_index]
            box = make_box(box_exact,
                           tp)
        else:
